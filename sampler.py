...
class DiffusionSampler(nn.Module):
    ...
    def sample(self, model, x_start, operator, measurement, SDE=False, record=False, verbose=False):
        """
...
class LangevinDynamics(nn.Module):
    ...
    def sample(self, x0hat,model, operator, measurement, sigma, ratio, record=False, verbose=False):
        """
        ...
        #input_tensor.data = x0hat.clone().to(device)
...
class DAPS(nn.Module):
    ...
    def sample(self, model, x_start, operator, measurement, evaluator=None, record=False, verbose=False, **kwargs):
        ...
        for step in pbar:
            ...
            x0hat = sampler.sample(model, xt,operator, measurement, SDE=False, verbose=False)

            # 2. langevin dynamics
            x0y = self.lgvd.sample(x0hat,model, operator, measurement, sigma, step / self.annealing_scheduler.num_steps)
