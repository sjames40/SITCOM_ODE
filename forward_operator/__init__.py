...
class Operator(ABC):
    ...
    def error(self, x, y):
        ...
        #print(f"measurement requires_grad: {y.requires_grad}")
        result = ((self(x) - y) ** 2).flatten(1).sum(-1)  # Your error computation
        #print(f"error result requires_grad: {result.requires_grad}")
        return result
...
class DownSampling(Operator):
    def __init__(self, resolution=256, scale_factor=4, device='cuda', sigma=0.01):
...
class GaussianBlur(Operator):
    def __init__(self, kernel_size, intensity, device='cuda', sigma=0.01):
        super().__init__(sigma)
        self.device = device
        self.kernel_size = kernel_size
...
class PhaseRetrieval(Operator):
    def __init__(self, oversample=2.0, resolution=256, sigma=0.01):
        super().__init__(sigma)
...
class NonlinearBlur(Operator):
    def __init__(self, opt_yml_path, device='cuda', sigma=0.01):
        super().__init__(sigma)
        self.device = device
    ...
    def prepare_nonlinear_blur_model(self, opt_yml_path):
        ...
        blur_model.load_state_dict(torch.load("/mnt/DataB/DAPS-main/forward_operator/bkse/experiments/pretrained/GOPRO_wVAE.pth"))
...
class HighDynamicRange(Operator):
    def __init__(self, device='cuda', scale=2, sigma=0.01):
        super().__init__(sigma)
        self.device = device
        self.scale = scale
